...
func convertJSCalEventToICal(event *jscal.Event) (*ics.VEvent, error) {
	...
	if event.Start != nil {
		if event.IsAllDay() {
			...
			// Timed event
			if event.TimeZone != nil && *event.TimeZone != "" && *event.TimeZone != "UTC" {
				// Set with timezone
				loc, err := time.LoadLocation(*event.TimeZone)
				if err == nil {
					vevent.SetStartAt(event.Start.In(loc))
				} else {
...
func formatRRule(rule *jscal.RecurrenceRule) string {
	...
	if rule.Count != nil {
		...
		parts = append(parts, fmt.Sprintf("UNTIL=%s", rule.Until.Format("20060102T150405Z")))
	}
